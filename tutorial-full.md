...
- **為什麼要做這個？** 建立網路防火牆。`demo-sg` 負責擋下外網不合法的請求；`demo-ecs-sg` 負責確保只有來自負載均衡器（ALB）的流量才能進入 ECS 容器，並且也要允許自己的流量。
...
![](images/1-6.png)
   ...
   - 點擊右下角 `Create security group`。
![](images/1-3.png)

- **[Security Group]** (點擊剛剛建立好的 `demo-ecs-sg)
	- 點擊`Edit inbound rules`
	- Source: 點選輸入框，尋找並選擇剛建立的 `demo-ecs-sg`
	- 完成：`Save rules`
![](images/1-13.png)
...
2. **[Task definition configuration]**
   ...
   - OS/Architecture: `Linux/ARM64`(如果 是 docker build --platform=linux/amd64 -> 這裡要選 Linux/x86_64)
