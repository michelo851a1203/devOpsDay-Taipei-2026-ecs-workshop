...
3. **配置參數**：
   ...
   - **[Infrastructure]**：僅勾選 `AWS Fargate (serverless)`。
   - **[Monitoring]**：開啟 `Container Insights` (用於收集 AIOps 決策所需的指標)。
4. **完成**：點擊右下角 `Create cluster`。

## ⑥ 設定任務定義 (Task Definition)

- **為什麼要做這個？** 撰寫容器的「規格說明書」。定義這個 Go 程式需要多少 CPU、記憶體、使用哪張映像檔，以及環境變數（區分 Blue/Green 版本）。
...
3. **[Container - 1]**
   ...
   - Name: `api`
   - Image URI: 貼上在 **ECR 複製的 Image URI**
...
5. **[Log collection]**：勾選 Use log collection。
...
3. 在 `[Permissions]` 頁籤中，點擊右側 `Add permissions` -> 選擇 `Attach policies`。
